...
#
# Kevin and Kevin's fair warning: this class has been insanely optimized for
# speed. It is intended to be immutable.
# Please don't modify attributes after instantiation. :)
#
...
class Range(object):
    ...
    def __init__(self, start, end, worksheet, validate=True):
        self._start = (Range.string_to_coordinate(start)
                       if validate and isinstance(start, six.string_types)
                       else start)
        self._end = (Range.string_to_coordinate(end)
                     if validate and isinstance(end, six.string_types)
                     else end)

        if ((not (1 <= self._start[0] <= 65536)
             and self._start[0] != float('inf'))
            or (not (1 <= self._end[0] <= 65536)
                and self._end[0] != float('inf'))):
            raise Exception("Row index out of bounds")

        if ((not (1 <= self._start[1] <= 256)
             and self._start[1] != float('inf'))
            or (not (1 <= self._end[1] <= 256)
        ...
        self.is_row = (self._end[1] == float('inf') and
                       self._start[0] == self._end[0] and
                       self._start[1] == 1)

        self.is_column = (self._end[0] == float('inf') and
                          self._start[1] == self._end[1] and
                          self._start[0] == 1)
        ...
        def __setattr__(self, name, value):
            if isinstance(self._parent, Style.Style):
                ...
                for cell in self._parent:
                    setattr(reduce(getattr, self._attribute.split('.'), cell),
                            name, value)
            ...
            if len(data) <= self.height:
                for row in data:
                    if len(row) > self.width:
                        raise Exception("Row too large for range, row has %s "
                                        "columns, but range only has %s" %
                                        (len(row), self.width))
                for x, row in enumerate(data):
                    for y, value in enumerate(row):
                        method(x + self._start[0], y + self._start[1], value)
            else:
                raise Exception("Too many rows for range, data has %s rows, "
                                "but range only has %s" %
    ...
    def __str__(self):
        return (Range.coordinate_to_string(self._start) + ":" +
