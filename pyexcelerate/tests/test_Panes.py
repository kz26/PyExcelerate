...
def test_panes_output():
    eq_(
        to_unicode('<pane state="frozen" topLeftCell="A2" ySplit="1"/>'),
        Panes(y=1).get_xml(),
    )
    eq_(
        to_unicode('<pane topLeftCell="B1" xSplit="1"/>'),
        Panes(x=1, freeze=False).get_xml(),
    )
    eq_(
        to_unicode('<pane state="frozen" topLeftCell="C3" xSplit="2" ySplit="2"/>'),
        Panes(x=2, y=2).get_xml(),
    )
