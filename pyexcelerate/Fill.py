...
class Fill(object):
    ...
    def get_xml_string(self):
        if not self.background:
            ...
            return "<fill><patternFill patternType=\"solid\">" \
                   "<fgColor rgb=\"%s\"/></patternFill></fill>" % \
                   self.background.hex

    def __or__(self, other):
        return Fill(
            background=Utility.nonboolean_or(
                self._background, other._background, None
            )
        )

    def __and__(self, other):
        return Fill(
            background=Utility.nonboolean_and(
                self._background, other._background, None
            )
        )
    ...
    def __xor__(self, other):
        return Fill(
            background=Utility.nonboolean_xor(
                self._background, other._background, None
            )
        )
